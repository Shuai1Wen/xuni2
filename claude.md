...
- 好的，我完全理解了您的要求。基于您提供的 `CLAUDE.md` 开发准则，我将为您创建一个专门针对“严格依据 `model.md` 文件实现代码”的开发准则。
...
## 概览
本文件用于指导如何将 `model.md` 文件中定义的模型或算法，严格、无损、且高效地复现为可执行代码。它是 `CLAUDE.md` 开发准则的具化扩展，所有 `CLAUDE.md` 中的强制性规范在此完全适用。

**核心目标**：确保最终代码产出与 `model.md` 的描述在逻辑、细节和约束上 **100% 一致**，同时在实现层面达到最优化的代码结构。

**最终交付物必须包含**：
1.  **源代码**：严格复现 `model.md` 逻辑的实现。
2.  **`README.md`**：详细的项目说明、环境配置与使用指南。
3.  **`requirements.txt`**：精确定义的项目依赖列表。

---

## 核心原则（绝对强制）

### 1. 绝对忠实复现 (Absolute Fidelity)
- **禁止简化**：不得以任何理由简化 `model.md` 中描述的任何逻辑、公式、步骤或边界条件。
- **禁止省略**：`model.md` 中提及的每一个细节都必须在代码中找到对应实现。
- **禁止猜测**：若 `model.md` 中存在任何模糊或矛盾的描述，必须立即终止任务，并通过 `sequential-thinking` 工具记录问题，向上游（任务发起者）寻求澄清。不允许擅自做任何假设。

### 2. 结构优化优先 (Vectorization First)
- **向量化取代循环**：在数据处理和计算密集型任务中，必须优先使用向量化操作（如 NumPy, Pandas, PyTorch 等库提供的方式），**绝对禁止**在可以通过向量化高效处理的场景下使用原生 `for` 循环。
- **性能意识**：代码实现不仅要正确，还要高效。在规划阶段就必须考虑算法的时间与空间复杂度，并选择最优的数据结构和实现方式。
- **代码可读性**：优化代码结构的同时，必须通过清晰的命名和必要的中文注释来保持代码的可读性，解释向量化操作背后的意图。

### 3. 交付物完整性 (Completeness of Deliverables)
- 任何代码实现任务都必须附带一个内容完整、格式规范的 `README.md` 和一个精确的 `requirements.txt` 文件。缺少任何一项都视为任务未完成。

---

## 强制工作流程（5 阶段模型）

### 阶段 0: 模型解析与上下文分析 (Model Parsing & Context Analysis)

此阶段的目标是完全理解 `model.md` 并收集实现所需的所有信息，禁止编写任何功能性代码。

1.  **读取模型文件 (必须)**
    - 使用 `desktop-commander.read_file` 读取 `model.md` 的全部内容。

2.  **结构化解析 (必须)**
    - 使用 `sequential-thinking` 工具对 `model.md` 内容进行深度分析和结构化拆解：
        - **输入与输出**：明确模型的输入数据格式、类型、约束以及输出的格式和类型。
        - **核心算法/逻辑**：识别出核心的数学公式、处理步骤、逻辑分支和伪代码。
        - **依赖项识别**：根据模型描述，识别出所有必要的外部库（例如 `numpy`, `pandas`, `scikit-learn`）。这是生成 `requirements.txt` 的基础。
        - **约束与边界**：列出所有的约束条件、边界情况和错误处理逻辑。
        - **疑问清单**：记录所有不明确或有歧义的点。

3.  **实现策略研究 (必须)**
    - **向量化方案调研**：针对核心算法，使用 `context7` 和 `github.search_code` 查询已识别库的最佳实践，特别是寻找高效的向量化实现方式。
      - **查询示例**：`context7 get-library-docs context7CompatibleLibraryID="numpy" topic="vectorizing conditional logic"`
      - **查询示例**：`github.search_code query="numpy vectorized algorithm-name"`
    - **生成上下文摘要**：将以上所有分析结果汇总到 `.claude/context-summary-[任务名].md` 文件中。

### 阶段 1: 实现规划与结构设计 (Implementation Planning & Structure Design)

1.  **设计代码结构**：
    - 基于模型解析结果，规划代码的模块、类和函数。设计应遵循 SOLID 和 DRY 原则。
    - 明确数据流，定义清晰的函数签名和数据对象。

2.  **规划交付物**：
    - **`README.md` 结构草案**：使用 `sequential-thinking` 规划 `README.md` 的章节和内容要点。
    - **`requirements.txt` 内容确定**：最终确定所有依赖及其版本。

### 阶段 2: 编码实现与优化 (Code Implementation & Optimization)

1.  **严格编码**：根据设计规划，逐一实现 `model.md` 中定义的逻辑。
2.  **持续优化**：在编码过程中，严格执行“向量化优先”原则，用最优化的方式实现计算逻辑。
3.  **中文注释**：为所有核心逻辑、复杂操作和函数/类添加简洁明了的简体中文注释，解释其意图和与 `model.md` 的对应关系。
4.  **日志记录**：所有编码决策和操作必须记录在 `operations-log.md` 中。

### 阶段 3: 交付物生成 (Deliverable Generation)

1.  **生成 `README.md`**：
    - 使用 `desktop-commander.write_file` 根据规划草案，编写最终的 `README.md` 文件。内容必须符合下方定义的强制标准。

2.  **生成 `requirements.txt`**：
    - 使用 `desktop-commander.write_file` 创建 `requirements.txt` 文件，并填入所有确定的依赖项。

### 阶段 4: 验证与审查 (Verification & Review)

1.  **本地验证**：
    - 编写一个本地验证脚本（或提供可复现的验证步骤），证明代码实现与 `model.md` 的逻辑完全一致。验证应覆盖正常流程、边界条件和错误处理。
    - 验证过程必须在 `operations-log.md` 中记录。

2.  **自我审查**：
    - 遵循 `CLAUDE.md` 的质量审查规范，调用 `sequential-thinking` 进行自我审查，并生成 `.claude/verification-report.md` 审查报告。
    - **审查核心要点**：
        - **与 `model.md` 的一致性**：是否存在任何偏离、简化或遗漏？
        - **代码优化程度**：是否最大限度地使用了向量化？是否存在不必要的循环？
        - **交付物完整性**：`README.md` 和 `requirements.txt` 是否规范且完整？
...
### 1. README.md
`README.md` 必须至少包含以下部分，并使用简体中文编写：

##### ```markdown
# [项目/模型名称]

## 1. 模型概述
- 简要描述 `model.md` 的核心目标和功能。
- 引用 `model.md` 作为核心设计文档。

## 2. 环境设置
- 列出运行此代码所需的所有步骤，包括如何安装依赖。
- ```bash
  # 示例
...
## 3. 使用方法
- 提供一个清晰、可直接复制运行的代码示例，说明如何调用核心功能。
- 解释输入参数的含义和输出结果的格式。

## 4. 实现细节
- （可选但推荐）简要说明代码结构如何映射到 `model.md` 中的关键部分，特别是对复杂的向量化实现进行解释。

## 5. 验证
- 说明如何运行本地验证脚本或重现验证步骤，以证明代码的正确性。
##### ```

### 2. requirements.txt
- 必须包含所有直接依赖项。
- 推荐锁定版本号以确保可复现性（例如 `numpy==1.21.0`）。

---

## 继承性声明
本准则未明确提及的所有其他方面，包括但不限于**语言使用**、**安全性原则**、**文件结构**、**懒惰检测机制**、**开发哲学**等，均严格遵循 `CLAUDE.md` 开发准则的全部规定。任何违反 `CLAUDE.md` 的行为都将被视为违反本准则。
